...
| `--guest-builds-dir` |         | Path to a directory in guest to use for storing builds, useful when mounting a block device to the VM (mutually exclusive with `--builds-dir`)             |
| `--guest-cache-dir`  |         | Path to a directory in guest to use for caching purposes, useful when mounting a block device to the VM (mutually exclusive with `--cache-dir`)            |
